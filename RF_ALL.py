...
file2 = "gatedAtnAll.csv"
file3 = "GatedBiAttentionOutput.csv"
...
dataset_clinical = np.loadtxt("A:/Project/Stacked Ensemble/Data/METABRIC_clinical_1980.txt", delimiter="\t") # Change the path to your local system
dataset_exp = np.loadtxt("A:/Project/Stacked Ensemble/Data/METABRIC_gene_exp_1980.txt", delimiter="\t")# Change the path to your local system
dataset_cnv = np.loadtxt("A:/Project/Stacked Ensemble/Data/METABRIC_cnv_1980.txt", delimiter="\t")# Change the path to your local system
...
for train_index, test_index in kfold.split(X_clinical, Y_clinical):
    ...
    x_train_clinical = np.expand_dims(x_train_clinical, axis=2)
    x_test_clinical = np.expand_dims(x_test_clinical, axis=2)
    ...
    x_train_exp = np.expand_dims(x_train_exp, axis=2)
    x_test_exp = np.expand_dims(x_test_exp, axis=2)
    ...
    x_train_cnv = np.expand_dims(x_train_cnv, axis=2)
    x_test_cnv = np.expand_dims(x_test_cnv, axis=2)
...
#print("Accuracy = %.2f%% (+/- %.2f%%)\n" % (np.mean(acc_cvscores), np.std(acc_cvscores)))
#print("Precision = %.2f%% (+/- %.2f%%)\n" % (np.mean(Pr_cvscores), np.std(Pr_cvscores)))
print("Sensitivity = %.2f%% (+/- %.2f%%)\n" % (np.mean(Sn_cvscores), np.std(Sn_cvscores)))
#print("Mcc = %.2f%% (+/- %.2f%%)\n" % (np.mean(Mcc_cvscores), np.std(Mcc_cvscores)))




X_clinical = np.expand_dims(X_clinical, axis=2)
X_exp = np.expand_dims(X_exp, axis=2)
X_cnv = np.expand_dims(X_cnv, axis=2)
...
scores2 = cross_val_score(rf, X2, Y2, cv=10,verbose=0)
scores3 = cross_val_score(rf, X3, Y3, cv=10,verbose=0)
...
predictions2 = cross_val_predict(rf, X2, Y2, cv=10,method='predict_proba')[:, 1]
predictions3 = cross_val_predict(rf, X3, Y3, cv=10,method='predict_proba')[:, 1]
y_pred = model.predict([X_clinical,X_exp,X_cnv])
fpr, tpr, thresholds = roc_curve(Y_clinical, y_pred,pos_label=1)
fpr1, tpr1, thresholds1 = roc_curve(Y1, predictions1,pos_label=1)
fpr2, tpr2, thresholds2 = roc_curve(Y2, predictions2,pos_label=1)
fpr3, tpr3, thresholds3 = roc_curve(Y3, predictions3,pos_label=1)
roc_auc = auc(fpr, tpr)
roc_auc1 = auc(fpr1, tpr1)
roc_auc2 = auc(fpr2, tpr2)
roc_auc3 = auc(fpr3, tpr3)

plt.plot(fpr,tpr, 'c', label = 'AGCNN Bi-Attention = %0.3f' %roc_auc)
plt.plot(fpr2,tpr2, 'b', label = 'AGCNN STACKED RF = %0.3f' %roc_auc2)
plt.plot(fpr3,tpr3, 'g', label = 'AGCNN Bi-Attention STACKED RF = %0.3f' %roc_auc3)
plt.plot(fpr1,tpr1, 'r', label = 'AGCNN + Input STACKED RF = %0.3f' %roc_auc1)
