...
print('*********************************Training the Clinical SiGaAtCNN *****************************************')
# kfold_value fold cross validation
kfold = StratifiedKFold(n_splits=kfold_value, shuffle=False, random_state=1)
Sn_cln_cvscores = []
i=1  
...
print('***************************************************** Training the EXPR SiGaAtCNN *****************************************************')
kfold = StratifiedKFold(n_splits=kfold_value, shuffle=False, random_state=1)   
Sn_exp_cvscores =[]
i=1
for train_index, test_index in kfold.split(X_exp, Y_exp):
...
print('***************************************************** Training the CNV SiGaAtCNN *****************************************************')
kfold = StratifiedKFold(n_splits=kfold_value, shuffle=False, random_state=1)
Sn_cnv_cvscores = []
i=1
for train_index, test_index in kfold.split(X_cnv, Y_cnv):
...
plt.plot(fpr_agcnnclinical, tpr_agcnnclinical, 'y', label = 'SiGaAtCNN-CLN = %0.3f' % roc_auc_agcnnclinical)
plt.plot(fpr_clinical, tpr_clinical, 'r', label = 'CNN-CLN = %0.3f' % roc_auc_clinical)

plt.plot(fpr_agcnncnv, tpr_agcnncnv, 'o', label = 'SiGaAtCNN-CNA = %0.3f' % roc_auc_agcnncnv)
plt.plot(fpr_cnv, tpr_cnv, 'b', label = 'CNN-CNA = %0.3f' % roc_auc_cnv)

plt.plot(fpr_agcnnexp, tpr_agcnnexp, 'c', label = 'SiGaAtCNN-EXPR = %0.3f' % roc_auc_agcnnexp)
